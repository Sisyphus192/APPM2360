...
x = 0:h:48;
y1 = 15000*exp(-0.2*x);
plot(x, abs(y1 - ode_euler(N_A_prime, 0:h:48, A_0)), 'Color', [243/255,132/255,0], 'LineWidth', 2)
...
x = 0:h:48;
y1 = 15000*exp(-0.2*x);
plot(0:h:48, abs(y1 - ode_euler(N_A_prime, 0:h:48, A_0)), 'Color', [161/255,202/255,241/255], 'LineWidth', 2)

h = 1
%plot(0:h:48, ode_euler(N_A_prime, 0:h:48, A_0), 'Color', [190/255,0,50/255], 'LineWidth', 2)
x = 0:h:48;
y1 = 15000*exp(-0.2*x);
plot(0:h:48, abs(y1 - ode_euler(N_A_prime, 0:h:48, A_0)), 'Color', [190/255,0,50/255], 'LineWidth', 2)


hold off
axis([0 48 1 600])
set(gca,'yscale','log')
legend('h=0.01 Error','h=0.1 Error','h=1 Error')
title("Absolute Error of Euler Methods")
ylabel('Absolute Error (# of Atoms)')
xlabel('t (seconds)')
axis tight
